...
class ErrorFreeSQLiVulnCheck(VulnCheck):
    NAME = "ErrorFreeSQLi"

    def __init__(self, oob_server_url="http://localhost:5001"):
        self.oob_server_url = oob_server_url

    def check(self, candidate):
        """
        Check OOB server for database side-effect detections
        """
        import requests
        
        try:
            # Query OOB server for this coverage_id
            check_url = f"{self.oob_server_url}/check/{candidate.coverage_id}"
            response = requests.get(check_url, timeout=2)
            
            if response.status_code == 200:
                data = response.json()
                if data.get('detected', False):
                    # Store side effects in candidate for reporting
                    candidate.side_effects = data.get('side_effects', {})
                    return True
        except Exception as e:
            # OOB server not available or error - skip detection
            pass
...
class ParamBasedVulnChecker(DefaultVulnChecker):
    def __init__(self, mysql_errors_folder=None, shell_errors_folder=None, unserialize_errors_folder=None, pathtraversal_errors_folder=None, xxe_errors_folder=None, oob_server_url="http://localhost:5001"):
        self.vuln_checkers = [
            WebFuzzXSSVulnCheck(),
            ParamBasedSQLiVulnCheck(mysql_errors_folder),
            ErrorFreeSQLiVulnCheck(oob_server_url),  # NEW: Error-free SQLi detection
